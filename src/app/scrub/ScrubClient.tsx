      ...
      return Object.entries(meta)
          ...
          .map(([key, value]) => (
              <div key={key} className="flex justify-between py-2 border-b border-border last:border-0">
                  <span className="text-muted-foreground text-sm font-mono">{key}</span>
            ...
            <div className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="space-y-6">
                    ...
                    {file.status === 'done' && file.scrubbedUrl && (
                     <div className="bg-primary/10 border border-primary/20 rounded-2xl p-6 text-center animate-in fade-in slide-in-from-bottom-4">
                            ...
                            <p className="text-muted-foreground mb-6 text-sm">All metadata has been removed successfully.</p>
                            <a 
                                href={file.scrubbedUrl} 
                                download={`scrubbed-${file.file.name}`}
                                className="inline-block bg-primary hover:bg-primary/90 text-white font-bold py-3 px-8 rounded-xl transition-all"
                            >
                                Download Safe Image
                            </a>
                        </div>
                ...
                <GlassCard className="h-fit max-h-[800px] overflow-hidden flex flex-col p-6">
                    <div className="flex items-center justify-between mb-6">
                        <h2 className="text-xl font-bold flex items-center gap-2 text-foreground">
                            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-primary"><circle cx="12" cy="12" r="10"/><line x1="12" y1="16" x2="12" y2="12"/><line x1="12" y1="8" x2="12.01" y2="8"/></svg>
                            Metadata Report
                        </h2>
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar mb-6">
                        {file.status === 'scanning' ? (
                            <div className="py-12 text-center">
                                <div className="w-8 h-8 rounded-full border border-primary border-t-transparent animate-spin mx-auto mb-4"></div>
                                <p className="text-muted-foreground">Scanning image data...</p>
                            </div>
                        ) : file.metadata ? (
                    ...
                    {file.status === 'scanned' && (
                        <div className="pt-6 border-t border-border animate-in fade-in slide-in-from-bottom-2">
                             ...
                             <button 
                                onClick={handleScrub}
                                className="w-full bg-primary hover:bg-primary/90 text-white font-bold py-4 rounded-xl transition-all flex items-center justify-center gap-2 text-lg shadow-sm hover:shadow-sm"
                             >
