...
const title = 'Favicon Generator - MicroPng'
const description =
  'Generate web, mobile, and legacy .ico icons from a single upload.'
const canonicalUrl = 'https://micropng.sahilfruitwala.com/favicon'
...
function FaviconPage() {
  ...
  const [generatedFiles, setGeneratedFiles] = useState<
    { size: number; blob: Blob; url: string; type: 'web' | 'mobile' | 'ico' }[]
  >([])
  ...
  const generateFavicons = async () => {
    ...
    try {
      ...
      const allPngSizes = Array.from(
        new Set([...FAVICON_SIZES, ...MOBILE_SIZES, ...ICO_SIZES]),
      )
      ...
      const pngPromises = allPngSizes.map(async (size) => {
        ...
        if (!response.ok) {
          throw new Error(`Failed to generate ${size}x${size} png`)
        }
    ...
    generatedFiles.forEach((file) => {
      const name =
        file.type === 'mobile'
          ...
          : `favicon-${file.size}x${file.size}.png`
              ...
              <div className="bg-secondary border border-border rounded-3xl p-8 flex flex-col md:flex-row gap-8 items-center">
                ...
                <div className="flex-1 text-center md:text-left">
                  ...
                  <p className="text-muted-foreground mb-6 font-medium">
                    Now generates Apple Touch Icon and favicon.ico
                  </p>
                  <button
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-4">
                      {generatedFiles
                        ...
                        .map((file) => (
                          <div
                            key={file.size}
                            className="bg-surface border border-border rounded-2xl p-4 flex flex-col items-center gap-3 transition-colors hover:border-primary/50 group"
                          >
                            <div className="w-12 h-12 flex items-center justify-center bg-black rounded-lg border border-border overflow-hidden">
                              <img
                                src={file.url}
                                alt={`${file.size}x${file.size}`}
