      ...
      POST: async ({ request }) => {
        try {
          ...
          const compressionLevel =
            (formData.get('level') as string) || 'mid' // 'best' | 'mid' | 'low' | 'lossless'
          ...
          const resizeFit =
            (formData.get('fit') as keyof sharp.FitEnum) || 'cover'

          // Manual Crop parameters (pixels)
          const cropX = formData.get('cropX')
            ? parseInt(formData.get('cropX') as string)
            : null
          const cropY = formData.get('cropY')
            ? parseInt(formData.get('cropY') as string)
            : null
          const cropW = formData.get('cropW')
            ? parseInt(formData.get('cropW') as string)
            : null
          const cropH = formData.get('cropH')
            ? parseInt(formData.get('cropH') as string)
            : null

          const finalWidth = widthRaw ? parseInt(widthRaw as string) : undefined
          const finalHeight = heightRaw
            ? parseInt(heightRaw as string)
            : undefined

          const shouldCrop =
            cropW !== null && cropH !== null && cropX !== null && cropY !== null
          const shouldResize =
            (finalWidth && !isNaN(finalWidth)) ||
            (finalHeight && !isNaN(finalHeight))
          ...
          // 4. Compress helper
          const compressBuffer = async (quality: number, isFast = false) => {
